...
layout(std140) uniform AirCurrents
{
    VelocityField currents[MAX_AIR_CURRENTS];
    vec3 gravity;
    uint numCurrents;
};
...
//Total field velocity at a point (superposition of all current contributions).
//Shared by the visualizer (.geom) and the advection (.comp) so they never disagree.
vec3 sampleVelocityField(vec3 p)
{
